...
from transformers.modeling_utils import PreTrainedModel #, find_pruneable_heads_and_indices, prune_conv1d_layer
...
#            self.warn_if_padding_and_no_attention_mask(input_ids, attention_mask)
        ...
        if getattr(self.config, "add_cross_attention", False) and encoder_hidden_states is not None:
        ...
        if head_mask is None:
            ...
            head_mask = head_mask.view(self.config.n_layer, -1, -1, -1, -1)
...
#        self.multiple_choice_head = SequenceSummary(config)
        self.multiple_choice_head = nn.Sequential(
            nn.Dropout(getattr(config, "summary_first_dropout", 0.1)),
            nn.Linear(config.n_embd, config.n_embd),
            nn.Tanh(),
            nn.Dropout(getattr(config, "summary_last_dropout", 0.1)),
    ...
    def forward(
        ...
        **kwargs,
        ...
        lm_logits = self.lm_head(hidden_states)
        cls_index = mc_token_ids.to(hidden_states.device)
        summary_input = hidden_states.gather(
            1, 
            cls_index.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, hidden_states.size(-1))
        ).squeeze(1)
        mc_logits = self.multiple_choice_head(summary_input).squeeze(-1)
