...
def main(args):
    ...
    directory = Path(f"./out/{args.batch}")
    for tokens_path in directory.glob("*.pt"):
        ...
        codes_vocal = tokens[1:2, :].unsqueeze(0)  # Shape: [B, 1, T]
        codes_bgm   = tokens[2:3, :].unsqueeze(0)  # Shape: [B, 1, T]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Script generating audio from tokens')
    parser.add_argument('--batch', type=str, required=True,
                      help='Basename of batch to generate audio for')
